  ...
  <div class="shop-info">
    <div class="info-content">
      ...
      <section class="section">
        ...
        <div class="activity">
          <div class="activity-item" :class="activity[support.type]" v-for="(support, index) in info.supports" :key="index">
...
export default {
  ...
  methods: {
    _initScorll () {
      ...
      new BScroll('.shop-info', {// 注册滚动组件,默认为垂直滑动
      })
      // 动态计算ul的宽度
      const ul = this.$refs.picsUr
      const liWidth = 120
      const space = 16
      const count = this.info.pics.length
      ul.style.width = (liWidth + space) * count - space + 'px'
      /* eslint-disable no-new */
      new BScroll('.pic-wrapper', {// 注册滚动组件
